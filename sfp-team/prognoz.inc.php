...
socket.on("scoredatas",function(d){if(sendfp){$.post("https://fprognoz.org/",{matches:JSON.stringify(d.data.matches),a:"sfp-team",m:"prognoz",l:"'.$l.'",s:"'.$s.'",t:"'.$t.'"},function(json){$.each(JSON.parse(json),function(idx,obj){base.push(obj.id);base[obj.id]=obj.d})})}$("#statusline").css("display","none")})
...
socket.on("guncelleme",function(d){var json="";$.each(d.updates,function(index,ux){if(base[ux.idx]!==undefined){if(ux.s==4&&base[ux.idx][3]!="FT")json+=(json.length?",":"")+JSON.stringify(ux);scorefix(ux)}});if(json.length)$.post("https://fprognoz.org/",{updates:"["+json+"]",a:"sfp-team",m:"prognoz",l:"'.$l.'",s:"'.$s.'",t:"'.$t.'"'.(isset($n)?',n:"'.$n.'"':'').'},function(html){$("#pl").html(html)})})
//]]></script>
