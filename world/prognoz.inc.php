...
$(document).ready(function(){
	$(".input_module input").each(function(){validate($(this).attr("id"),$(this).val())})
	$(".input_module input").change(function(){validate($(this).attr("id"),$(this).val())})
	$(".input_module input").on("input",function(){validate($(this).attr("id"),$(this).val())})
...
';
  ...
  foreach ($roles as $league => $coach) if ($coach == 'coach' && ($league == 's' || $tour < 'UNL12' || $tour > 'UNL94' ))
  {
...
//<input type="hidden" id="tour" name="tour" value="'.$tour.'">
    $out .= '
<br>
<div class="ps-3 h6">Управление сборной в '.($tour < 'UNL12' || $tour > 'UNL94' ? 'Лиге '.($league == 'n' ? 'Наций' : 'Сайтов') : ($tour < 'UNL17' ? 'Финальном турнире' : 'Турнире ЧЕ 2021')).'</div>
...
<button class="btn btn-primary ms-3 mb-3" id="save_squad_'.$league.'" onClick="sendRoster(\''.$league.'\');return false">сохранить</button>
';
//</form>
  }
  return $out;
...
'Гости успешно завершают атаку! игрок гостей повернулся и пробил своей коронной и единственной левой.',
'Мяч, преодолев последнее препятствие между ногами вратаря, влетает в ворота.',
'Вот это гол, вот это гол, голешник!',
'Мяч в сетке! Плачет от счастья главный тренер гостей... Нет, это просто кто-то из его помощников попал ему пальцем в глаз...',
'игрок гостей разводит мяч и вратаря по разным углам, как супружескую пару на суде!',
       ],
 -1 => [
'Нападающие гостей прописались в штрафной хозяев. игрок хозяев лукаво не мудрствует - мяч в аут - из сердца вон!',
...
'Гости громят, давят и производит другие насильственные меры над номинальными хозявами поля.',
'игрок гостей постоянно маячит на острие атаки.',
'Гости всем составом сидят, висят на воротах соперника.',
'Редким по красоте ударом, без подготовки, игрок хозяев уложил нападающего на землю.',
'Выйдя на мяч, вратарь опередил защитника и выбил его за боковую.',
'Мяч не хочет идти в ворота, как его ни упрашивают...',
       ],
  0 => [
'Мяч подпрыгнул и обманул нападающего. Хозяева отодвигают его в центр поля.',
'Задушат, задавят, зацелуют, - подумал игрок гостей и решительно пробил мимо ворот.',
...
'Прострельная передача! игрок гостей совсем не успел на нее.',
'игрок гостей хотел пробить сильно и точно, но не получилось... Да, неудачно лег на ногу футболисту его кожаный дружок.',
'игрок хозяев сорвал замыслы противников и аплодисменты зрителей.',
'И вот мяч попадает в оператора за воротами хозяев! Это очень понравилось взыскательным болельщикам.',
       ],
  1 => [
'Хозяева проводят контратаку. Ну, будем считать этот эпизод опасным моментом.',
'Пас через все поле, который сделал игрок хозяев оказался не очень точным. Угловой в исполнении хозяев.',
'игрок хозяев убежал в сольный проход, но не сумел создать опасный момент и ждёт подкрепление на подходе к штрафной гостей.',
'игрок хозяев делает длинный и точный пас поближе к штрафной гостей. Может получиться опасный момент.',
'Разряжая ситуацию, игрок хозяев сильно бьет в сторону ворот соперника.',
       ],
  2 => [
...
'Это был очень коварный для вратаря удар. Ведь мяч летел по <b>траектории</b>!',
'игрок гостей издали перебросил мяч через вратаря, увидев, что тот далеко вышел из ворот поговорить с защитниками.',
'Мяч влетел в специально для этого пpиготовленные воpота.',
       ],
 -1 => [
'игрок хозяев делает безадресный пас. Мяч попадает точно в ноги нападающему гостей.',
...
'игрок гостей обвел одного, другого, опять другого, и снова его же...',
'Как говорится, с песнями бежал игрок гостей с центра поля, оставляя за спиной, как березки, соперников.',
'Все сделал игрок гостей, только ударить забыл... ну и немудрено - столько бежать, имя свое забыть можно...',
'игрок гостей смог в углу центрального круга отобрать мяч у кого-то из соперников.',
'игрок гостей нащупал изъян между ног у соперника и ловко им воспользовался.',
       ],
  0 => [
...
'Идут целенаправленные передачи в рекламные щиты',
'Обе команды очень хорошо играют... без мяча',
'Удар по ногам, арбитр останавливает встречу и показывает, что бить можно только по свистку. игрок хозяев сам виноват: отдал бы мяч - не получил бы по ногам.',
'игрок гостей в великолепном дриблинге теряет мяч.',
'Боковой аpбитp пpинимает кpасивые позы. Возможно, он pаньше занимался балетом.',
'Нельзя, нельзя обыгрывать партнера стоя на месте! Это надо делать в движении!',
'Мяч в правом углу центрального круга.',
       ],
  1 => [
'игрок гостей делает безадресный пас. Мяч попадает точно в ноги нападающему хозяев.',
...
'игрок хозяев обвел одного, другого, опять другого, и снова его же...',
'Как говорится, с песнями бежал игрок хозяев с центра поля, оставляя за спиной, как березки, соперников.',
'Все сделал игрок хозяев, только ударить забыл... ну и немудрено - столько бежать, имя свое забыть можно...',
'игрок хозяев смог в углу центрального круга отобрать мяч у кого-то из соперников.',
'игрок хозяев нащупал изъян между ног у соперника и ловко им воспользовался.',
       ],
  2 => [
...
'Это был очень коварный для вратаря удар. Ведь мяч летел по <b>траектории</b>!',
'игрок хозяев издали перебросил мяч через вратаря, увидев, что тот далеко вышел из ворот поговорить с защитниками.',
'Мяч влетел в специально для этого пpиготовленные воpота.',
       ],
 ),
1 => array(
...
'игрок гостей делает удивительный забег, итогом которого становится точный удар из штрафной.',
 ...
 -1 => [
'Гости проводят контратаку. Ну, будем считать этот эпизод опасным моментом.',
'Пас через все поле, который сделал игрок гостей оказался не очень точным. Угловой в исполнении гостей.',
'игрок гостей убежал в сольный проход, но не сумел создать опасный момент и ждёт подкрепление на подходе к штрафной хозяев.',
'игрок гостей делает длинный и точный пас поближе к штрафной хозяев. Может получиться опасный момент.',
'Разряжая ситуацию, игрок гостей сильно бьет в сторону ворот соперника.',
       ],
  0 => [
'Мяч подпрыгнул и обманул нападающего. Гости отодвигают его в центр поля.',
'Задушат, задавят, зацелуют, - подумал игрок хозяев и решительно пробил мимо ворот.',
...
'Прострельная передача! игрок хозяев совсем не успел на нее.',
'игрок хозяев хотел пробить сильно и точно, но не получилось... Да, неудачно лег на ногу футболисту его кожаный дружок.',
'игрок гостей сорвал замыслы противников и аплодисменты зрителей.',
'И вот мяч попадает в оператора за воротами гостей! Это очень понравилось взыскательным болельщикам.',
       ],
  1 => [
'Нападающие хозяев прописались в штрафной гостей. игрок гостей лукаво не мудрствует - мяч в аут - из сердца вон!',
...
'Хозева громят, давят и производит другие насильственные меры над командой гостей.',
'игрок хозяев постоянно маячит на острие атаки.',
'Хозяева всем составом сидят, висят на воротах соперника.',
'Редким по красоте ударом, без подготовки, игрок гостей уложил нападающего на землю.',
'Выйдя на мяч, вратарь опередил защитника и выбил его за боковую.',
'Мяч не хочет идти в ворота, как его ни упрашивают...',
       ],
  2 => [
'Этот гол напрашивался.',
'Вынимай! Вины голкипера в пропущенном мяче нет.',
'Хозяева успешно завершают атаку! игрок хозяев повернулся и пробил своей коронной и единственной левой.',
'Мяч, преодолев последнее препятствие между ногами вратаря, влетает в ворота.',
'Вот это гол, вот это гол, голешник!',
'Мяч в сетке! Плачет от счастья главный тренер хозяев... Нет, это просто кто-то из его помощников попал ему пальцем в глаз...',
'игрок хозяев разводит мяч и вратаря по разным углам, как супружескую пару на суде!',
       ],
 )
);
...
function ball_form($nm, $match, $home, $away, $prognoz_half, $prognoz_array) {
  ...
  <div class="time">
    <div class="row">
      <div class="col-3 small text-start">победа<br>хозяев</div>
      <div class="col-6 header_b">Первый тайм</div>
      <div class="col-3 small text-end">победа<br>гостей</div>
    </div>';
...
if (isset($_SESSION['Coach_name']))
{
    ...
    if (in_array($teamCodes['n'],
        [
//            'Vano Opulsky',
            'Олег Крупич',
        ]))
        $teamCodes['s'] = $teamCodes['n'];
  ...
  if (count($teamCodes))
  {
    ...
    if (($t < 12 || $t > 94) && isset($teamCodes['n']) && is_file($team_select))
    {
      // определение роли
...
if ($t < 12)
  ...
  $head .= 'Лига Наций '.$s.'. Пробный тур '.($t - 94);
...
{
  $head .= '
<span class="small">тур: <b>'.$cca.$t.'</b> ';
  if ($t > 11 && $t < 95 || count($teamCodes) == 1 || $teamCodes['n'] == $teamCodes['s'])
...
';
    ...
    foreach ($virtmatches as $matchn => $teams)
    {
      ...
      if (($t < 12 || $t > 94) && $teams['home']['team'] != $home_team_fn)
      {
        $league = 'n';
        $size = 5;
      }
...
if (!$published)
  ...
  {
    if (($t < 12 || $t > 94 || $league == 's') && $roles[$league] == 'coach') {
      if (is_file($prognoz_dir.'/.'.$cc[$league])) {
      ...
      $hint .= '<a href="" onClick=\'$("#chronology-'.$league.'").toggle(); return false\'>показать/скрыть историю тура в '.(($t < 12 || $t > 94) ? 'Лиге '.($league == 's' ? 'Сайтов' : 'Наций&nbsp;') : 'Финальном турнире').'</a><br>';
      $hint .= '<p id="chronology-'.$league.'" class="text-start" style="display:none">';
...
{
  $html = $closed ? '' : '
<link rel="stylesheet" href="/css/balls.css?ver=18">';
  $html .= '
<script>//<![CDATA[';
  $html .= $published ? '' : '
...
<div class="d-flex">
	<div id="statusline" class="w-100 text-start">получение результатов с <a href="https://www.livescore.bz" sport="football(soccer)" data-1="today" lang="en">www.livescore.bz</a></div>
	<div id="timedisplay" class="w-100 text-end">'.$notice.'</div>
</div>';
  $html .= '
...
<div id="dynamic" class="dynamic" data-tab="' . ($n ?? 1) . '">
	<div class="col order-2">Матчи тура:' . ($cal ?? '') . '</div>
	<div id="pl" class="col order-1" tabindex="-1" style="min-width:576px;">' . $prognozlist . '</div>
</div>
