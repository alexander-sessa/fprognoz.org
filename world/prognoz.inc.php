      ...
      for ($i=0; $i<6; $i++) {
        ...
        if ($i == 2) {
          ...
          if ($half2)
            ...
            $prognozColored .= ' <input type="checkbox" name="'.rawurlencode($name).'" '.($repl ? 'checked="checked"' : '').' />  ';
          else
  ...
  if ($team_code) {
    ...
    if (isset($_POST['replace'])) {
      ...
      foreach ($lines as $line) if (trim($line)) {
        ...
        $log .= (strlen($log) > 45 ? ', ' : ' ') . (isset($_POST[rawurlencode($name)]) ? '<b>' : '') . $name . (isset($_POST[rawurlencode($name)]) ? '</b>' : '');
        $out .= $name.';'.$predict.';'.trim($ts).';'.(isset($_POST[rawurlencode($name)]) ? '4' : '').'
';
      }
      $cc_pr = explode("\n", $out);
