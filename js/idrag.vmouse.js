...
jQuery.iDrag =	{
	...
	destroy : function()
	{
		return this.each(
			...
			{
				if (this.isDraggable) {
					this.dragCfg.dhe.off('vmousedown', jQuery.iDrag.draginit);
					this.dragCfg = null;
	...
	draginit : function (e)
	{
		...
		jQuery(document)
			.on('vmousemove', jQuery.iDrag.dragmove)
			.on('vmouseup', jQuery.iDrag.dragstop);
	...
	dragstart : function(e)
	{
		...
		if (elm.dragCfg.insideParent || elm.dragCfg.containment == 'parent') {
			...
			elm.dragCfg.oC.x = elm.offsetLeft + parentBorders.l;
			elm.dragCfg.oC.y = elm.offsetTop + parentBorders.t;
	...
	dragstop : function(e)
	{
		...
		jQuery(document)
			.off('vmousemove', jQuery.iDrag.dragmove)
			.off('vmouseup', jQuery.iDrag.dragstop);
	...
	build : function(o)
	{
		...
		return this.each(
			function()
			{
				...
				dhe.on('vmousedown', jQuery.iDrag.draginit);
			}
		)
	}
};
