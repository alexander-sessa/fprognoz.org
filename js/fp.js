...
function passwordCheck(str){
  $.post("/online/ajax.php",{data:$("#pass_str").data("tpl"),nick:$("#name_str").val(),pswd:str},function(r){
    if(r=='0'){
      $("#valid_name").html('<span style="color:lightsalmon"><i class="fas fa-times" /> неверный пароль</span>');
      fg=setTimeout(function(){$("#forget").show()},17000)
    }
    else if(r=='1'){
      $("#valid_name").html('<span style="color:lightgreen"><i class="fas fa-check" /> добро пожаловать!</span>');
      $("#l_form").submit()}
    else if(r=='2'){
...
$("#pass_str").keyup(function(k){
  if($("#pass_str").is(":focus")&&k.key!="Shift"){
    ...
    if(str.length)passwordCheck(str);
      ...
      else $("#valid_name").html('<span style="color:lightgreen"><i class="fas fa-check" /> теперь введите пароль</span>')
    }
  }
})
