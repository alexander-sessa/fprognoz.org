...
//  }
  $c_form .= '
      ...
      <div class="c-submit">
        <button class="btn btn-secondary" onClick="$(\'#ta_%CKEDID%\').val($(\'#%CKEDID%\').html())">Комментировать</button>
      </div>
    </footer>
    </form>' . ($hidden ? '' : '
...
<div id="comment" style="background:white;margin-top:10px;padding-top:5px;text-align:left;width:100%">
  <div style="text-align:center">
    <h5>' . ($coach_name ? 'Здесь Вы можете оставить свой комментарий' : 'Для полного доступа необходимо авторизоваться на сайте') . '</h5>
  </div>'
