...
function build_itogi($country_code, $season, $tour)
{
  ...
  if (is_file($online_dir."$country_code/$season/programms/$tour"))
  {
    ...
    $itfname = $online_dir."$country_code/$season/publish/$tcode/itc".substr($tour, -2);
    $rfname = $online_dir."$country_code/$season/publish/$tcode/rc".substr($tour, -2);
    ...
    $bfname = $online_dir."$country_code/$season/$tcode/bombc/b".substr($tour, -2);
...
////////  $prognoz[$rep+1] = '=';
    ...
    $line1 = sprintf('%'.(14 + $ppos).'s', $podstr);
...
//        $tn = ' '.$realmatch[$nm]['trnr'];
    ...
    for ($tn=1; $tn<=$tourn; $tn++)
    {
      ...
      $virtmatch = array();
      $ngroup = 0;
//      if (($tcode != 'UEFAL' && sizeof($matches) >= 8) || sizeof($matches) >= 16)
      if (sizeof($matches) > 88)
      {
        ...
        { // формирование составов групп по первому туру с числом матчей >= 8
          ...
          $ateams[$i][substr($line, 0, strpos($line, ' - '))] = 0;
          $ateams[$i][substr($line, strpos($line, ' - ') + 3)] = 0;
          $line = $matches[2 * $i + 1];
          $line = substr($line, 0, strpos($line, '  '));
          $ateams[$i][substr($line, 0, strpos($line, ' - '))] = 0;
          $ateams[$i][substr($line, strpos($line, ' - ') + 3)] = 0;
      ...
      $amb = array();
      if (strpos($matches[0],':')) foreach ($matches as $line) if ($line = trim($line))
      {
...
// bw bwh bwa bgh bga bsh bsa bl blh bla lgh lga lsh lsa wag was ser h hh ha
    ...
    $out = '';
    reset($wresults);
    $rprognoztxt = 'Прав. прогноз';
    $rprognoztxtln = 13;
//$ngroups = 0;
    if ($ngroups) for ($gn=0; $gn<$ngroups; $gn++)
    {
      $out .= group_table($acal, $ateams, $gn, $tl);
      $out .= $rprognoztxt . str_repeat(' ', $tl - $rprognoztxtln) . $rprognoz . '    Счёт
';
      for ($i=0; $i<6; $i++)
        $out .= $wresults[$gn * 6 + $i] . "\n";
    ...
    else
    {
      $out .= $rprognoztxt . str_repeat(' ', $tl - $rprognoztxtln) . $rprognoz . '    Счёт
';
      foreach ($wresults as $line)
        $out .= "$line\n";

    }
    $template = str_replace('[EuroResults]', $out, $template);
    if ($maxlname)
      $nmax = $maxlname + 2;
    else
      $nmax = $maxteam + 2;
...
//      $out .= sprintf('%-'.$maxln.'s', $att['h'][$i].' - '.$att['a'][$i])
  ...
  {
    $att = array();
    foreach ($gb as $nm => $atemp)
    {
      $tr = $gb[$nm]['n'];
      $att['t'][] = $tr;
      $att['n'][] = $nm;
      $att['h'][] = $gb[$nm]['h'];
      $att['z'][] = sizeof($gb[$nm]['t']);
      $att['pl'][] = $tt[$nm]['pl'];
      $att['p'][] = $tt[$nm]['p'];
      $att['r'][] = $tt[$nm]['r'];
      $att['g'][] = $tt[$nm]['g'];
    ...
    for($i=0; $i<sizeof($att['t']); $i++) if ($nm = trim($att['n'][$i]))
    {
      $gb[$nm]['p'] = $i + 1;
      $out .= sprintf('%2s', $i+1).'.  '
            . $gb[$nm]['n'] . str_repeat(' ', $maxcoach + 1 - mb_strlen($gb[$nm]['n']))
            . $nm . str_repeat(' ', $maxteam + 1 - mb_strlen($nm))
            . sprintf('%3s', $att['h'][$i]);
...
if (isset($_SESSION['Country_code']) && isset($_SESSION['Coach_name']) && isset($_SESSION['Session_password']))
{
  ...
  //////////////////////////////// SENDER
    ...
    file_put_contents($file, str_replace("\r", "", $_POST['msgtext']));
...
//          send_email($senders[$country_code], $ateams[2], $ateams[3], $_POST['subject'], $_POST['msgtext']);
      ...
      if ($emails = rtrim($emails, ', '))
        send_email($senders[$country_code], '', $emails, $_POST['subject'], $_POST['msgtext']);
...
//    echo "<br /><a href=# onClick=history.back();>Назад</a>";
  ...
  //////////////////////////////// EDITOR
    ...
    if (isset($_GET['file']))
    {
      ...
      {
        $body = build_itogi($cca, $s, $l.strtoupper($t));
        sleep(1);
        $file = $online_dir."$cca/$s/publish/$l/itc".strtolower($t);
        if ($t[0] == 'G') $subject = " Итоги золотого матча";
        else $subject = ' Итоги '.ltrim($t, '0CPGS').'-го тура';
      }
      else if ($file == 'review')
      {
        $file = $online_dir."$cca/$s/publish/$l/rc".strtolower($t);
        if ($t[0] == 'G') $subject = " Обзор золотого матча";
        else $subject = ' Обзор '.ltrim($t, '0CPGS').'-го тура';
      }
      else if ($file == 'prognoz')
      {
        $file = $online_dir."$cca/$s/publish/$l/$cca".strtoupper($t);
//        if (is_file($file))
...
SU.FOOTBALL.PROGNOZ <input type="checkbox" name="sendnews" disabled="disabled" />
</td></tr>
<tr><td>Тема:</td><td><input name="subject" value="'.$subjects[$country_code].$subject.'" size="80" />
<img src="images/spacer.gif" width="82" height="1" alt="" />
<input type="submit" name="sendmail" value="Отправить" /></td></tr>
<tr><td>Текст:</td><td><textarea name="msgtext" rows="'.$rows.'" cols="100">'.$body.'</textarea></td></tr>
</table>
</form>
';
  }
